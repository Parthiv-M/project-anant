        ...
        if (loggedIn) {
            try {
                ...
                const res = await axios.get(
                    `${process.env.NEXT_PUBLIC_SERVER_URL}/downloadtopology/?id=${mxeneIds}`
                );
                ...
                MyToaster({
                    ...
                    message: `You have downloaded ${idList.length} mxene${idList.length === 1 ? "" : "s"
                        }`,
                });
                setIdList([]);
            } catch (error) {
            ...
            <div className="w-screen min-h-screen pt-16 flex flex-col items-center justify-start">
                ...
                <div className="w-full px-8 py-6">
                    <div className="flex flex-row mb-5 gap-2 items-center">
                        ...
                        {idList.length <= 0 && res.topologyMxenes.length > 0 && (
                            <p className="text-gray-300">
                                Displaying{" "}
                                <span className="underline font-bold">
                                    {20 * parseInt(res.currentPage - 1) + 1} -{" "}
                                    {20 * parseInt(res.currentPage - 1) + res.topologyMxenes.length}
                                </span>{" "}
                                of <span className="font-bold">{res.totalResults}</span> mxenes
                            </p>
                        )}
                    </div>
                    {res.topologyMxenes.length > 0 ? (
                        <Fragment>
                            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                                {res.topologyMxenes.map((topMxene) => {
                                    return (
                                        topMxene.mxene !== null &&
                                        <ResultCard
                                            key={topMxene.mxene.id}
                                            id={topMxene.id}
                                            mxene={topMxene.mxene.mxene}
                                            cardType={"topology"}
                                            latticeConstant={topMxene.latticeConstant ? topMxene.latticeConstant : topMxene.mxene.latticeConstant}
                                            bandGap={topMxene.socBandGap ? topMxene.socBandGap : topMxene.mxene.bandGap}
                                            idList={idList}
                                            setIdList={setIdList}
...
export async function getServerSideProps(context) {
    ...
    if (redirectLink) {
        ...
        const fetchURL = process.env.NEXT_PUBLIC_SERVER_URL + "/searchtopology";
